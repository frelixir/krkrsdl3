...
import static org.tvp.krkrsdl3.MainActivity.SHAREDPREF_GAMEURI;
...
public class KRKRActivity extends SDLActivity {
    ...
    public void playVideo(byte[] pathbyte) {
        ...
        try {
            ...
            path = path.replace('\\', '/');
            if(m_onsbase==null)  {
                // scoped storage, content://org.tvp.krkrsdl.fileProvider/external_file_path/
            ...
            Log.i("## krkrsdl_android", "playVideo " + uri.toString());
            invokeSystemPlayer(uri);

        } catch (Exception e) {
            Log.e("## krkrsdl_android", "playVideo error:  " + e.getClass().getName() + " "+ e.getMessage());
        }
    }

    private void invokeSystemPlayer(Uri uri){
        AlertDialog.Builder builder = new AlertDialog.Builder(KRKRActivity.this);
