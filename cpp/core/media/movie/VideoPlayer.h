...
struct CCurrentStream
{
    ...
    CCurrentStream(AVMediaType t, int i) : type(t), player(i) { Clear(); }
...
public:
    ...
    SelectionStream& Get(AVMediaType type, int index);
    bool Get(AVMediaType type, int flag, SelectionStream& out);

    SelectionStreams Get(AVMediaType type);
    template<typename Compare>
    SelectionStreams Get(AVMediaType type, Compare compare)
    {
...
class BasePlayer : public tTVPThread
{
public:
    void Play();
    void Stop();
...
public:
    ...
    virtual ~BasePlayer();
    bool OpenFromStream(tTJSBinaryStream* stream,
    ...
    bool OpenStream(CCurrentStream& current, int iStream, bool reset = true);
    ...
    bool CheckIsCurrent(CCurrentStream& current, AVStream* stream, DemuxPacket* pkg);
    void ProcessPacket(AVStream* pStream, DemuxPacket* pPacket);
    void ProcessAudioData(AVStream* pStream, DemuxPacket* pPacket);
    void ProcessVideoData(AVStream* pStream, DemuxPacket* pPacket);
    ...
    double GetSpeed();
    void FrameMove() { m_VideoPlayerVideo->FrameMove(); }
    bool IsStopped() { return GetTerminated(); }
    ...
    bool ReadPacket(DemuxPacket*& packet, AVStream*& stream);
    bool IsValidStream(CCurrentStream& stream);
    bool IsBetterStream(CCurrentStream& current, AVStream* stream);
    void CheckBetterStream(CCurrentStream& current, AVStream* stream);
    void CheckStreamChanges(CCurrentStream& current, AVStream* stream);
    //	bool CheckDelayedChannelEntry(void);

    void OpenInputStream(tTJSBinaryStream* stream, const std::string& filename);
...
private:
    ...
    TVPElapsedTimer m_cachingTimer;
    TVPElapsedTimer m_ChannelEntryTimeOut;
    ...
    TVPElapsedTimer m_player_status_timer;
