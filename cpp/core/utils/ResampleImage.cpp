...
static inline void AxisParamCalculateWeight(
    float* weight, float*& output, int& len, int leftedge, int rightedge)
{
...
static void AxisParamCalculateAxis(TParam& param,
                                   int srcstart,
                                   int srcend,
                                   int srclength,
                                   int dstlength,
...
static void AxisParamCalculateAxisAreaAvg(
    TParam& param, int srcstart, int srcend, int srclength, int dstlength)
{
    if (dstlength <= srclength)
    { // 縮小のみ
        TVPCalculateAxisAreaAvg(srcstart, srcend, srclength, dstlength, param.start_, param.length_,
                                param.weight_);
...
public:
    ...
    /** 縦方向の拡大縮小処理 */
    inline void samplingVertical(int y,
                                 tjs_uint32* dstbits,
                                 int dstheight,
                                 int srcwidth,
    ...
    /** 横方向の拡大縮小処理 */
    inline void samplingHorizontal(tjs_uint32* dstbits,
                                   const int offsetx,
                                   const int dstwidth,
                       ...
                       const tTVPRect& srcrect)
    {
        ...
        tjs_uint32* dstbits =
            (tjs_uint32*)dest->GetScanLineForWrite(clip.dst_top_) + clip.dst_left_;
        if (blendfunc == NULL)
        {
...
public:
                  ...
                  TWeightFunc& func)
    {
        ...
        AxisParamCalculateAxis(paramy_, srcrect.top, srcrect.bottom, srcheight, dstheight, tap,
                               func);
...
void ResamplerFunc(void* p)
{
    ...
    tjs_uint32* workbits = &work[0];
    tjs_int dststride = dest->GetPitchBytes() / (int)sizeof(tjs_uint32);
    tjs_uint32* dstbits = (tjs_uint32*)dest->GetScanLineForWrite(param->start_ + destrect.top) +
                          param->clip_->dst_left_;
    if (param->blendfunc_ == NULL)
    {
        for (int y = param->start_; y < param->end_; y++)
        {
            param->sampler_->samplingVertical(y, workbits, dstheight, srcwidth, src, srcrect,
                                              wstarty);
                        ...
                        float sharpness)
{
    ...
    Resampler sampler;
    sampler.ResampleMT(clip, blendfunc, dest, destrect, src, srcrect, BicubicWeight::RANGE,
                       weightfunc);
}
                       ...
                       const tTVPRect& srcrect)
{
    ...
    Resampler sampler;
    sampler.ResampleMT(clip, blendfunc, dest, destrect, src, srcrect, TWeightFunc::RANGE,
                       weightfunc);
}
 ...
 */
void TVPResampleImage(const tTVPRect& cliprect,
                      iTVPBaseBitmap* dest,
                      const tTVPRect& destrect,
                      const iTVPBaseBitmap* src,
                      const tTVPRect& srcrect,
